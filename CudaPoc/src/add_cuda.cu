...
namespace cuda_poc
{
    ...
    template void vector_add<float2>(float2* c, const float2* a, const float2* b, size_t n, dim3 grid_dim,
                                     dim3 block_dim);
    template void vector_add<float3>(float3* c, const float3* a, const float3* b, size_t n, dim3 grid_dim,
                                     dim3 block_dim);
    template void vector_add<float4>(float4* c, const float4* a, const float4* b, size_t n, dim3 grid_dim,
                                     dim3 block_dim);
