...
template void linear_v1<float>(int M, int N, int K, float alpha, const float* A, const float* B, float beta, float* C);
...
template void linear_v2<float>(int M, int N, int K, float alpha, const float* A, const float* B, float beta, float* C);
...
//
// cd C:\Ws\CudaPoc\CudaPoc\src; $vsPath = 'C:\Program Files\Microsoft Visual
// Studio\2022\Enterprise\VC\Auxiliary\Build\vcvars64.bat'; cmd /c "`"$vsPath`" && nvcc --ptxas-options=-v -c
// gemm_cuda.cu 2>&1"
// **********************************************************************
...
#pragma unroll
    for (int m = 0; m < THREAD_M; ++m) {
      ...
      int a_col = bk + threadIdx.x;
      As[threadIdx.y + m * BLOCK_DIM][threadIdx.x] = (a_row < M && a_col < K) ? A[a_row * K + a_col] : T(0);
    }
...
template void linear_v3<float>(int M, int N, int K, float alpha, const float* A, const float* B, float beta, float* C);
...
#pragma unroll
    for (int n = 0; n < THREAD_N; ++n) {
      ...
      Bs[threadIdx.y][threadIdx.x + n * BLOCK_DIM] = (b_row < K && b_col < N) ? B[b_row * N + b_col] : T(0);
    }
...
template void linear_v4<float>(int M, int N, int K, float alpha, const float* A, const float* B, float beta, float* C);
...
template void linear_v5<float>(int M, int N, int K, float alpha, const float* A, const float* B, float beta, float* C);
...
void linear_v6(cublasHandle_t handle, int M, int N, int K, T alpha, const T* A, const T* B, T beta, T* C) {
  ...
  cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K,  // Swapped dimensions
              &alpha, B, N,                               // B becomes first operand with leading dimension N
              A, K,                                       // A becomes second operand with leading dimension K
              &beta, C, N);                               // C with leading dimension N
}

template void linear_v6<float>(cublasHandle_t handle,
                               int M,
                               int N,
                               int K,
                               float alpha,
...
template void linear_v7<float>(int M, int N, int K, float alpha, const float* A, const float* B, float beta, float* C);
}  // namespace cuda_poc::linear
