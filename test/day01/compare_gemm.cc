...
//
// === Kernel: gemm_v7_cute (M=4096, N=2048, K=256) ===
// Execution time: 298.877 ms
// Grid dimensions: (128, 64, 1)
// Block dimensions: (32, 32, 1)
// Total threads: 8388608
// GPU memory used: 10 MB
// GPU memory free: 3224.25 MB / 4095.56 MB
...
void test_gemm_with_kernel_fun(
    const char* test_name,
    int M,
    int N,
    int K,
    std::function<void(int, int, int, float, const float*, const float*, float, float*)> gemm_kernel_func) {
...
TEST_F(CudaPoc_Day0401, GemmV6_4096x2048x256) {
  ...
  auto gemm_func = std::bind(linear_v6<float>, handle, std::placeholders::_1, std::placeholders::_2,
...
TEST_F(CudaPoc_Day0401, GemmV7_4096x2048x256) {
  test_gemm_with_kernel_fun("gemm_v7_cute (M=4096, N=2048, K=256)", 4096, 2048, 256, linear_v7<float>);
}
}  // namespace cuda_poc::day01
